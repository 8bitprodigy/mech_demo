...
func _physics_process(delta):
	...
	angular_velocity.x = clamp(lerp(angular_velocity.x,input.rotation_vector.x/200,delta*ACCELERATION),-0.1, 0.1)
	angular_velocity.y = clamp(lerp(angular_velocity.y,input.rotation_vector.y/200,delta*ACCELERATION),-0.1, 0.1)
	...
	prints("Camera X ROT: ", $turret/Camera3D.rotation.x)
